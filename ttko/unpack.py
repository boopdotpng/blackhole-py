...
class Unpack:
  ...
  def _write_mode(self, engine, input_format, output_format, target, tilize,
    ...
    k = self.k
    output_format = output_format.register_format
    # E4M3 mode is shared with pack configuration; update only its own bit.
    reg = 71 if engine == UNPACKER0 else 119
    self._issue(TT.TTRMWCIB2(0x40, 0x40 if input_format is DType.FP8 else 0, reg))
    descriptor_x_dim = (
      x_dim if x_dim is not None else
      1024 if tilize else 0 if engine == UNPACKER0 else 256
    )
    descriptor = (
      input_format.hw_format | 0x10 | descriptor_x_dim << 16,
      1 | (1 if tilize else 4) << 16, 0, 0,
    )
    shift = 2 * input_format.itemsize
    word0 = 0x20 | (input_format.hw_format if input_format.is_fp8 else output_format)
  ...
  def move_matmul(self, left_cb, right_cb, *, right_transpose=False):
    ...
    if left_cb.dtype != right_cb.dtype or left_cb.dtype not in (DType.BF16, DType.FP8):
      raise ValueError("matmul unpack requires matching BF16 or FP8 inputs")
  ...
  def prepare_l1_pair_formats(self, dtype, source_b_dtype=None):
    """Set formats and invariant MOP state before a BF16 CB/L1 pair loop.

    Calls inside that loop may pass configure_format=False; all address and
    source-bank synchronization remains in move_l1_pair.
    """
    source_b_dtype = dtype if source_b_dtype is None else source_b_dtype
    self._wait_config_idle()
    self._set_thread_cfg(0, 0)
    self._write_mode(UNPACKER0, dtype, dtype, UnpackTarget.SRCA,
                     False, None)
    self._write_mode(UNPACKER1, source_b_dtype, source_b_dtype, UnpackTarget.SRCB,
                     False, None, x_dim=256)
    self._mop.configure(_PAIR_MOP)
    self._issue(TT.TTSETADCXX(1, 255, 0))
    self._issue(TT.TTSETADCXX(2, 255, 0))
    return self

  def move_l1_pair(self, source_cb, l1_address, *, configure_format=True,
                   tile_offset=0, pop=True, source_b_dtype=None):
    """Unpack a streamed tile and persistent operand; optionally retain a row.

    Prefill reuses a buffered weight row across several activation rows. The
    caller releases that row with CB.pop_front after its last use.
    """
    if type(tile_offset) is not int or not 0 <= tile_offset < source_cb.depth:
      raise ValueError("tile offset is outside the circular buffer")
    if pop and tile_offset:
      raise ValueError("offset unpack requires explicit row release")
    CB.wait_front(self.k, source_cb, tile_offset + 1)
    stall(self.k, Stall.UNPACK, Wait.SRCA_CLR | Wait.SRCB_CLR)
    if tile_offset:
      with self.k.scope():
        address, delta, limit = self.k.reg(3)
        CB.get_read_ptr(self.k, source_cb, address)
        self.k.li(delta, tile_offset * source_cb.tile_size)
        self.k.add(address, address, delta)
        self.k.li(limit, source_cb.limit)
        ready = self.k._new_label("retained_tile")
        self.k.bltu(address, limit, ready)
        self.k.li(delta, source_cb.size)
        self.k.sub(address, address, delta)
        self.k.label(ready)
        self._configure_l1(source_cb.dtype, UnpackTarget.SRCA, address, 256,
          commit=False, configure_mop=False, configure_format=configure_format)
    else:
      self._configure(
    ...
    self._configure_l1(
      source_cb.dtype if source_b_dtype is None else source_b_dtype, UnpackTarget.SRCB, l1_address, 256,
      commit=False, configure_mop=False, configure_format=configure_format,
    )
    self._commit_config(_BASE[UNPACKER0])
    if configure_format:
    ...
    stall(self.k, Stall.UNPACK, Wait.UNPACK0 | Wait.UNPACK1)
    sem_get(self.k, Sem.UNPACK_SYNC); sync(self.k)
    if pop: CB.pop_front(self.k, source_cb)
    return self
