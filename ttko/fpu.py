...
class Fpu:
  ...
  def matmul(self, *, dst_tile, accumulate=False, right_transpose=False, fidelity=2):
    if fidelity not in (1, 2): raise ValueError("matmul fidelity must be 1 or 2")
    self.dst.require_fp32()
    self._wait_for_dst()
    self._configure_dst(dst_tile)
