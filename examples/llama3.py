...
Weights retain their exact checkpoint row-major bytes, including the tied
embedding/LM head. Dense projection inputs use the same raw page order: the
ordinary unpacker applies the same lane permutation to both dot operands, so
the existing FPU multiply/SFPU reduction needs no tilize operation. RMSNorm is
also permutation invariant. Gather/scatter boundaries address dense vectors
explicitly; compact outputs and attention keep their existing internal layout.
