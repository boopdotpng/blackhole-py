...
def append_report(
  ...
  with path.open("a", encoding="utf-8") as f:
    f.write(f"\n## Run {now}\n\n")
    f.write(f"- Bytes per core: `{bytes_per_core}`\n")
    f.write(f"- Packet size: `{packet_bytes}` bytes\n")
    f.write(f"- DRAM banks/controllers used by allocator: `{bank_count}`\n")
    f.write(f"- Endpoint mode: `{endpoint_mode}`\n")
    f.write(f"- Posting mode: `{'stateful' if stateful else 'standard'}`\n")
    f.write("- Timing: device-side wall clock around NoC tile reads/writes and completion flush/barrier\n\n")
    f.write("\n".join(lines))
    f.write("\n")
