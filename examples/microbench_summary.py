...
Extracts all 'Run ...' sections from the four bench docs and computes
min/median/max adj-cyc/op across roles (or role+group for contention).

Usage:
    python examples/microbench_summary.py
    python examples/microbench_summary.py --latest          # newest run only
    python examples/microbench_summary.py --outliers        # cross-role spread >5%
    python examples/microbench_summary.py --files docs/riscv-core-microbench.md
"""
