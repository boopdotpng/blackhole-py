...
Brackets denote one resident launch. Sixteen layers use 83 launches per token.
  ...
  for (weight, _, local_rows), row_start, rotation in zip(
    ...
    if rotation is None:
      for local_row in p.brisc.range(local_rows):
        ...
        with p.brisc.scope():
          tiles = p.brisc.reg(input_tiles, exclude=source_row)
          p.brisc.slli(tiles[0], source_row, input_tiles.bit_length() - 1)
          for index in range(1, input_tiles):
            p.brisc.addi(tiles[index], tiles[0], index)
          p.brisc.noc_at(read_noc).read_tiles_into_cb(weight, tiles, weight_cb)
    else:
      ...
      with p.brisc.scope():
        ...
        if row_start is not None:
          with p.brisc.scope():
            ...
            p.brisc.slli(start, start, input_tiles.bit_length() - 1)
...
def _decode_projections_program(
  ...
  if norm_weight is None:
    ...
    if input_dim != EMBED_DIM: raise ValueError("fused RMSNorm requires a 2048-element token")
    if hybrid_rmsnorm_enabled():
      ...
      for buffer in (token, norm_weight):
        p.brisc.noc_at(read_noc).read_tiles_into_cb(buffer, (0, 1), operands)
      ...
      p.pack.move_tiles(normalized_cb, tiles=(0, 1))
    # Keep the rounded BF16 token in local L1 for all projection rows.
    CB.wait_front(p.brisc, normalized_cb, 2)
  ...
  *, query, head, key_cache=None, value_cache=None,
  ...
  with p.brisc.scope():
    ...
    if cos.tilized:
      ...
      p.brisc.slli(table_row_offset, table_row_offset, 7)
    ...
    for table_l1, operand_tile in ((cos_tile_l1, 2), (sin_tile_l1, 3)):
      segments = ((0, 0), (512, 512), (32, 32), (544, 544)) if cos.tilized else (
        (0, 0), (32, 512), (64, 32), (96, 544),
      )
      for source_delta, target_offset in segments:
        with p.brisc.scope():
...
def _prepare_attention_rope(p, inputs, key_cache, value_cache, start_pos, head, query_cb, group_size):
  ...
  with p.brisc.scope():
    ...
    p.brisc.read(position, p.param_addr(start_pos))
    p.brisc.srli(tile, position, 4)
    p.brisc.andi(row, position, 15)
    p.brisc.slli(row, row, 6)
    if cos.tilized:
      # A table token is two logical rows, or 64 BF16 elements.
      with p.brisc.scope():
      ...
      with p.brisc.scope():
        ...
        for i, (source_tile, slot) in enumerate(locations):
          ...
          with p.brisc.scope():
            ...
            p.brisc.write(operands.addr + _bf16_tile_byte_offset(group * 64 + i), value, bytes=2)
            if i >= 32: p.brisc.xor(value, value, sign)
            rotated = group * 64 + (i + 32 if i < 32 else i - 32)
        ...
        for i in (0, 16, 32, 48):
          with p.brisc.scope():
            ...
            l1.copy_words(p.brisc, table,
                          operands.addr + operand * operands.tile_size + _bf16_tile_byte_offset(group * 64 + i),
    ...
    for half in range(2):
      for face in range(2):
        l1.copy_words(p.ncrisc,
                      result.addr + _bf16_tile_byte_offset(group * 64 + half * 32 + face * 16),
                      query_cb.addr + half * query_cb.tile_size + _bf16_tile_byte_offset(group * 32 + face * 16),
                      8, unroll=8)
  _append_cache_rows(p, result.addr + _bf16_tile_byte_offset(group_size * 64), v_head,
...
def gqa_attention_fused(q, key_cache, value_cache, context, *, rope_inputs=None, attention_cores=8):
...
def _gqa_attention_program(
  ...
  *, rope_inputs=None, head_index=None, group_size=4,
  ...
  query_cb = p.cb(DType.BF16, depth=2)
  ...
  if rope_inputs is None:
    _zero_l1_words(p.brisc, query_low_l1, q.tile_size // 4)
    _zero_l1_words(p.brisc, query_high_l1, q.tile_size // 4)
  with p.brisc.scope():
    ...
    if rope_inputs is None:
      with p.brisc.noc.transaction() as transaction:
        for group_row in range(group_size):
          with p.brisc.scope():
            ...
            p.brisc.slli(query_head, query_head, group_shift)
      ...
      # Q is invariant across KV blocks. Initialize the ring once, then
      # republish the same immutable tiles as the consumer advances.
      for pair in range(query_cb.depth // 2):
        low = query_cb.addr + pair * 2 * query_cb.tile_size
        high = low + query_cb.tile_size
        l1.copy_words(p.brisc, query_low_l1, low, q.tile_size // 4, unroll=8)
        l1.copy_words(p.brisc, query_high_l1, high, q.tile_size // 4, unroll=8)
  ...
  with p.ncrisc.scope():
    head = p.ncrisc.reg()
    if head_index is None: p.ncrisc.read(head, p.param_addr(kv_head))
    else: p.ncrisc.li(head, head_index)
    ...
    with p.ncrisc.scope():
      ...
      p.ncrisc.srli(target_tile, head, 4 - group_shift)
      target_address, target_coordinate = p.ncrisc.noc._dram_tile(
        context, target_tile,
      )
      with p.ncrisc.noc.transaction() as transaction:
          ...
          for feature_half, source_base in enumerate((source0, source1)):
            ...
            for face_delta in (0, 512):
              with p.ncrisc.scope():
                ...
                p.ncrisc.andi(target_offset, head, (16 // group_size) - 1)
                p.ncrisc.slli(target_offset, target_offset, group_shift + 1)
                if logical_row:
                ...
                if context.tilized:
                  ...
                  p.ncrisc.slli(target_offset, target_offset, 6)
                if face_delta:
                  p.ncrisc.addi(target_offset, target_offset, 512 if context.tilized else 32)
...
class Llama3Decode:
  ...
  def __init__(self, safetensor_path="weights/llama3-1b/model.safetensors",
               device_index=0, *, attention_cores=16):
    if attention_cores not in (8, 16, 32): raise ValueError("attention cores must be 8, 16, or 32")
  ...
  def _allocate(self):
    ...
    def projection_weight(name, dtype, shape, axis=0, cores=cores):
      # Storage is the exact checkpoint byte stream; cores only own row ranges.
      storage = global_buffer(name, dtype, shape, axis, tilized=False)
      return replace(storage, cores=cores)
...
def run_decode_e2e(
  ...
  safetensor_path="weights/llama3-1b/model.safetensors",
  tokenizer_path="weights/llama3-1b",
  profile=False,
  device_index=0,
  attention_cores=16,
):
...
if __name__ == "__main__":
  ...
  parser.add_argument("--tokenizer", default="weights/llama3-1b")
