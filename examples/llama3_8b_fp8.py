...
from fw.consts import CQConfig, TensixL1, TensixMMIO
...
LLAMA_CORES = int(os.environ.get("LLAMA_PROJECTION_CORES", "96"))
if LLAMA_CORES not in (80, 88, 96, 104, 112, 117):
  raise ValueError("LLAMA_PROJECTION_CORES must be 80, 88, 96, 104, 112, or 117")
WEIGHT_DTYPE = DType.FP8 if os.environ.get("LLAMA_WEIGHT_DTYPE", "fp8") == "fp8" else DType.BF16
ACTIVATION_DTYPE = DType.FP8
ATTENTION_DTYPE = ACTIVATION_DTYPE if os.environ.get("LLAMA_ATTENTION_DTYPE", "bf16") == "fp8" else DType.BF16
INTERMEDIATE_DTYPE = ACTIVATION_DTYPE if WEIGHT_DTYPE.is_fp8 else DType.BF16
FP8_FIDELITY = int(os.environ.get("LLAMA_FP8_FIDELITY", "1"))
if FP8_FIDELITY not in (1, 2): raise ValueError("LLAMA_FP8_FIDELITY must be 1 or 2")
LLAMA_LAYERS = 32
PROJECTION_NOC_SPLIT_X = int(os.environ.get("LLAMA_NOC_SPLIT_X", "10"))
...
def _rmsnorm_one_token(sfpu, *, fp8=False, scale=None):
  ...
  if fp8:
    for tile in range(EMBEDDING_TILES): sfpu.map(_round_fp8_program(), tile=tile)
  sfpu.publish()


def _round_fp8_program(dtype=None):
  """Round FP32 mantissas to E4M3 before the truncating hardware packer."""
  dtype = ACTIVATION_DTYPE if dtype is None else dtype
  shift = 20
  maximum = 0x43e00000
  setup = []
...
def _scale_constants(name, value):
  return tuple(Const(f"{name}_{part}", word) for part, word in enumerate(_sfpu_float_words(LReg.L6, value)))


def _load_scale(p, name):
...
def _scale_input(p, tiles):
  _load_scale(p, "input_scale")
  words = [TT.TTSFPLOAD(LReg.L0, SfpuFormat.FP32, 7, 0)]
  _sfpu_mul(words, LReg.L0, LReg.L6, LReg.L0)
...
def _projection_dot_math(p, projections, input_tiles):
  ...
  fpu._mop.configure(LoopTemplate(
    outer=FP8_FIDELITY if projections[0][0].dtype is DType.FP8 else 2, inner=2, loop=TT.TTELWMUL(0, 0, 0, 0, 0),
  ...
  for projection_index, (_, _, local_rows) in enumerate(projections):
    for _ in p.trisc1.range(local_rows):
      ...
      if projections[0][0].dtype.is_fp8:
        _load_scale(p, f"output_scale_{projection_index}")
        words = []
        _sfpu_mul(words, LReg.L0, LReg.L6, LReg.L0)
...
def _decode_projections_program(
  ...
  p = Program(
    ...
    *(_scale_constants("input_scale", 1.) if projections[0][0].dtype.is_fp8 else ()),
    *(param for i in range(len(projections)) for param in (_scale_constants(f"output_scale_{i}", 1/256) if projections[0][0].dtype.is_fp8 else ())),
  ...
  operand_dtype = ACTIVATION_DTYPE if weight_cb.dtype.is_fp8 else DType.BF16
  ...
  if norm_weight is None and convert_input:
    ...
    for _ in p.trisc1.range(input_tiles):
      ...
      _scale_input(p, (0,))
      p.sfpu.map(_round_fp8_program(), tile=0).publish()
    ...
    _rmsnorm_one_token(p.sfpu, fp8=operand_dtype.is_fp8,
                       scale=(lambda: _scale_input(p, range(EMBEDDING_TILES))) if operand_dtype.is_fp8 else None)
  ...
  for target_l1, (_, output, local_rows) in zip(
    ...
    for local_row in p.ncrisc.range(local_rows):
      ...
      with p.ncrisc.scope():
        ...
        _tile_offset(p.ncrisc, local_row, byte_offset, scalar_dtype)
  ...
  if swiglu_output is not None:
    ...
    _swiglu_math(p, fp8=swiglu_output.dtype.is_fp8)
    ...
    _scatter_dense(p, compact_cbs[0], dense_output, projections[0][2], read_noc=1-read_noc, eth_output=eth_output)
  return p
...
def _swiglu_math(p, *, fp8=False):
  ...
  if fp8: p.sfpu.map(_round_fp8_program(), tile=0)
  p.sfpu.publish()
...
def _scatter_dense(p, cb, output, count, *, read_noc=0, eth_output=None):
  ...
  with k.scope():
    ...
    for index in k.range(count):
      with k.scope():
        ...
        _tile_offset(k, index, src, output.dtype)
    ...
    with k.scope():
      ...
      with k.scope():
        ...
        if eth_output is None:
          ...
          _add_constant(k, address, eth_output[2])
  ...
  if eth_output is not None:
    ...
    with k.scope():
      ...
      _add_constant(k, source, flag)
      k.write(source, sequence)
      k.slli(target, worker, 2)
      _add_constant(k, target, eth_output[3])
...
def _dense_offset(k, buffer, index, output):
  if buffer.tilized:
    _tile_offset(k, index, output, buffer.dtype)
  else:
    k.slli(output, index, buffer.dtype.itemsize.bit_length() - 1)


def _tile_offset(k, index, output, dtype=DType.BF16):
...
def _prepare_attention_rope(p, inputs, key_cache, value_cache, start_pos, head, query_cb, group_size):
  ...
  packed_offset = lambda index: _bf16_tile_byte_offset(index) // (2 if fp8 else 1)
  ...
  staging_tiles = max(20, math.ceil(HEAD_DIM / min(_token_counts(KV_PROJ_DIM, LLAMA_CORES))) + 1)
  ...
  if fp8:
    for tile in (0, 4): p.sfpu.map(_round_fp8_program(), tile=tile)
  p.sfpu.publish()
    ...
    for half in range(KV_CACHE_FEATURE_TILES):
      for face in range(2):
        l1.copy_words(p.ncrisc,
                      result.addr + packed_offset(group * HEAD_DIM + half * 32 + face * 16),
                      query_cb.addr + half * query_cb.tile_size + packed_offset(group * 32 + face * 16),
                      4 if fp8 else 8, unroll=4 if fp8 else 8)
  _append_cache_rows(p, result.addr + packed_offset(group_size * HEAD_DIM), result.addr + result.tile_size if fp8 else v_head,
...
def _gqa_attention_program(
  ...
  *, rope_inputs=None, head_index=None, group_size=4,
  ...
  query_cb = p.cb(key_cache.dtype, depth=4)
  key_cb = p.cb(key_cache.dtype, depth=8)
  value_cb = p.cb(key_cache.dtype, depth=8)
  probability_cb = p.cb(key_cache.dtype, depth=4)
  mask_cb = p.cb(DType.F32, depth=1)
  zero_cb = p.cb(DType.F32, depth=1)
  context_cb = p.cb(context.dtype, depth=4)
  ...
  with p.trisc1.scope():
    ...
    for block in p.trisc1.range(block_count):
      for feature in range(KV_CACHE_FEATURE_TILES):
        p.fpu.matmul(dst_tile=0, accumulate=feature != 0, right_transpose=True, fidelity=FP8_FIDELITY if key_cache.dtype.is_fp8 else 2)
      ...
      if key_cache.dtype.is_fp8: p.sfpu.map(_round_fp8_program(), tile=0)
      p.sfpu.publish()
      for tile in range(1, 5):
        p.fpu.matmul(dst_tile=tile, accumulate=True, fidelity=FP8_FIDELITY if key_cache.dtype.is_fp8 else 2)
    ...
    if context.dtype.is_fp8:
      for tile in (1, 2, 3, 4): p.sfpu.map(_round_fp8_program(), tile=tile)
    p.sfpu.publish()
        ...
        with p.ncrisc.noc.transaction() as transaction:
          for face in range(2):
            with p.ncrisc.scope():
              ...
              transaction.write(
                context_cb.addr + feature * context_cb.tile_size + _bf16_tile_byte_offset(group_row * 32 + face * 16) * context.dtype.itemsize // 2,
...
class Llama3Decode:
  ...
  def __init__(self, safetensor_path="weights/llama3-8b-fp8",
               device_index=1, *, attention_cores=32):
    if attention_cores not in (8, 16, 32): raise ValueError("attention cores must be 8, 16, or 32")
    self.attention_cores = attention_cores
    from llama_checkpoint import validate_checkpoint
    checkpoint = validate_checkpoint(safetensor_path)
    self.published_fp8 = "model.layers.0.self_attn.q_proj.input_scale" in checkpoint.tensors
    if not self.published_fp8:
      raise ValueError("use the published calibrated FP8 checkpoint; experimental conversion is retired")
    self.checkpoint_scales = {}
    if self.published_fp8:
      if WEIGHT_DTYPE is not DType.FP8: raise ValueError("published FP8 checkpoint requires FP8 weights")
  ...
  def _allocate(self):
    ...
    self.lm_storage = lm_storage = global_buffer("e2e_lm_storage", DType.BF16 if getattr(self, "published_fp8", False) else WEIGHT_DTYPE, (VOCAB_SIZE, EMBED_DIM), tilized=False)
    ...
    self.context = global_buffer(
      "e2e_context", DType.BF16 if getattr(self, "published_fp8", False) else INTERMEDIATE_DTYPE, GQA_CONTEXT_SHAPE,
      tilized=False,
    )
    mlp_compact_shape = (
    ...
    self.hidden_dense = global_buffer(
      "e2e_hidden_dense", DType.BF16 if getattr(self, "published_fp8", False) else INTERMEDIATE_DTYPE, (1, MLP_DIM),
      tilized=False,
    )
    ...
    for layer in range(LLAMA_LAYERS):
      ...
      weights = {
        ...
        "q": weight_buffer(
          f"{prefix}_q", WEIGHT_DTYPE, (Q_PROJ_DIM, EMBED_DIM),
          axis=0, cores=cores,
        ),
        "k": weight_buffer(
          f"{prefix}_k", WEIGHT_DTYPE, (KV_PROJ_DIM, EMBED_DIM),
          axis=0, cores=cores,
        ),
        "v": weight_buffer(
          f"{prefix}_v", WEIGHT_DTYPE, (KV_PROJ_DIM, EMBED_DIM),
          axis=0, cores=cores,
        ),
        "o": weight_buffer(
          f"{prefix}_o", WEIGHT_DTYPE, (EMBED_DIM, Q_PROJ_DIM),
          axis=0, cores=cores,
        ),
        "gate": weight_buffer(
          f"{prefix}_gate", WEIGHT_DTYPE, (MLP_DIM, EMBED_DIM),
          axis=0, cores=cores,
        ),
        "up": weight_buffer(
          f"{prefix}_up", WEIGHT_DTYPE, (MLP_DIM, EMBED_DIM),
          axis=0, cores=cores,
        ),
        "down": weight_buffer(
          f"{prefix}_down", WEIGHT_DTYPE, (EMBED_DIM, MLP_DIM),
          axis=0, cores=cores,
        ),
      }
      key_cache = global_buffer(
        f"{prefix}_key_cache", ATTENTION_DTYPE, KV_CACHE_STORAGE_SHAPE,
      )
      value_cache = global_buffer(
        f"{prefix}_value_cache", ATTENTION_DTYPE, KV_CACHE_STORAGE_SHAPE,
      )
      self.layers.append({
        "weights": weights,
        "key_cache": key_cache,
  ...
  def _upload_weights(self):
    ...
    cache_zeros = bytes(
      math.prod(KV_CACHE_STORAGE_SHAPE) * ATTENTION_DTYPE.itemsize,
    )
  ...
  def _projection_scales(self, layer, names):
    ...
    return {f"{name}_{part}": word for name, value in values.items() for part, word in enumerate(_sfpu_float_words(LReg.L6, value))}
...
def run_decode_e2e(
  ...
  safetensor_path="weights/llama3-8b-fp8",
  tokenizer_path="weights/llama3-8b-fp8",
  profile=False,
  device_index=1,
  attention_cores=32,
):
...
if __name__ == "__main__":
  ...
  parser.add_argument("--safetensor", default="weights/llama3-8b-fp8")
  parser.add_argument("--tokenizer", default="weights/llama3-8b-fp8")
