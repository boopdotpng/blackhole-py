...
from .mailbox import BriscMailbox as BM, NcriscMailbox as NM, TriscLocalMem as TLM, TriscMailbox
from .asm import KernelBase, A_DONE
from fw.consts import TensixL1 as LaunchL1
from .noc import Noc, NOC
from .cb import Cb, CB as CBRegs
from .tensix import Tensix
class Debug: pass
TILE = 32
...
def MOP_REPLAY(start_idx: int, length: int) -> int:
  ...
  return TTREPLAY(start_idx, length)
...
def _output_wait_ready(fw, val=t1):
...
def _output_reg(fw, offset, value, tmp=t1):
...
def emit_output_write_stateful(fw: MatmulKernel, src, dst_lo, dst_coord) -> MatmulKernel:
  ...
  fw.lw(a5, R.GP, 0x248)
  fw.bgeu(a5, t6, issue)
  _output_wait_ready(fw, val=a5)
  _output_reg(fw, 0x00, src, tmp=a5)
  _output_reg(fw, 0x0c, dst_lo, tmp=a5)
  _output_reg(fw, 0x14, dst_coord, tmp=a5)
  return _output_reg(fw, 0x40, NOC.CTRL_SEND_REQ, tmp=a5)
...
def emit_profile_stamp(fw: KernelBase, addr: int):
  if fw.role.startswith('trisc') and addr in (PROFILE_TRISC0, PROFILE_TRISC1, PROFILE_TRISC2):
    fw.csrrs(t1, zero, 0x7c0)
    fw.write32(addr + 16, t1)
  fw.read32(t1, TensixMMIO.RISCV_DEBUG_REG_WALL_CLOCK_L, tmp_addr=t0)
  fw.write32(addr, t1, tmp_addr=t0)
  fw.read32(t1, TensixMMIO.RISCV_DEBUG_REG_WALL_CLOCK_H, tmp_addr=t0)
  fw.write32(addr + 4, t1, tmp_addr=t0)
  return fw
...
def matmul_reader(plan: MatmulPlan) -> MatmulKernel:
  ...
  fw.read32(t1, LaunchL1.GRID_RANK_BASE + 4)
  sender, done = fw._new_label('a_sender'), fw._new_label('a_done')
  fw.beq(t1, zero, sender)
  emit_reader_recv(fw)
  fw.j(done)
  fw.label(sender)
...
def matmul_writer(plan: MatmulPlan) -> MatmulKernel:
  ...
  fw.read32(t1, LaunchL1.GRID_RANK_BASE)
...
def emit_reader_sender(fw: MatmulKernel, plan: MatmulPlan) -> MatmulKernel:
  ...
  fw.add(a6, a6, s3)
  fw.addi(s5,s5,-1)
  fw.bne(s5,zero,'reader_tile_rows')
...
def emit_writer_sender(fw: MatmulKernel, plan: MatmulPlan) -> MatmulKernel:
  ...
  fw.mv(a6, s1)
  fw.li(s5, plan.in0_block_w)
  fw.label('writer_tile_rows')
  for col in range(plan.per_core_n):
    fw.mv(a0, s0)
    _move_plus_imm(fw, a1, a6, col)
    fw.arg(a2, 29)
    fw.dram_tile_addr_from(NM.DRAM_BANK_TO_NOC_XY, a2, tile_bytes=INPUT_TILE_BYTES)
  ...
  fw.add(a6, a6, s3)
  fw.addi(s5,s5,-1)
  fw.bne(s5,zero,'writer_tile_rows')
...
def emit_output_writer(fw: MatmulKernel, plan: MatmulPlan) -> MatmulKernel:
  ...
  fw.read32(t3, LaunchL1.PARAM_BASE + 16)
  selected = fw._new_label('output_noc_selected')
  fw.li(t4, 2)
  fw.bne(t3, t4, selected)
  fw.read32(t3, LaunchL1.GRID_RANK_BASE + 4)
  fw.andi(t3, t3, 1)
  fw.xori(t3, t3, 1)
  fw.label(selected)
  fw.slli(R.GP, t3, 16)
  fw.li(t0, NOC.REGS_START_ADDR)
  fw.add(R.GP, R.GP, t0)
  ready = fw._new_label('output_noc_ready')
  fw.bne(t3, zero, ready)
  fw.li(t3, 1)
  fw.wait_sync_value(A_DONE, t3, actual=t4)
  fw.label(ready)
...
def emit_trisc0_unpack_subblock_reg(
  ...
  *, in0_block_base_tiles: int = 0, in1_block_base_tiles: int = 0,
  ...
  for inner in range(plan.in0_block_w):
    ...
    skip_tail(s6, plan.k_extent, plan.in0_block_w*32, inner, skip_inner)
    ...
    for row in range(plan.out_subblock_h):
      ...
      skip_tail(s4, plan.m_extent, plan.out_subblock_h*32, row, skip_row)
      ...
      if tail and tail_tiles != plan.out_subblock_w:
        ...
        fw.li(t0,full)
        fw.blt(s5,t0,normal)
        emit_trisc0_unpack_row_reg(fw,s9,s10,mop_loop_count=tail_tiles-1)
        fw.j(skip_row)
        fw.label(normal)
      emit_trisc0_unpack_row_reg(fw,s9,s10,mop_loop_count=plan.out_subblock_w-1)
      fw.label(skip_row)
    fw.label(skip_inner)
  return fw

def emit_trisc0_reload_subblock(fw: MatmulTrisc, plan: MatmulPlan) -> MatmulTrisc:
  if INPUT_DTYPE == Dtype.Float8_e4m3:
    fw.tensix_sync(0)
...
def _math_mop_cfg(plan: MatmulPlan) -> MopCfg:
  if EXPERIMENTAL_THROTTLE0:
    ...
    if INPUT_DTYPE == Dtype.Float8_e4m3:
      ...
      return replace(config, loop_inner=1)
    return config
...
def emit_math_dst_base_addr(fw: MatmulTrisc, out_reg=t1) -> MatmulTrisc:
  ...
  fw.li(t2, TTSETC16(ThreadCfg.DEST_TARGET_REG_CFG_MATH_Offset, 0))  # base; addr bits added in
...
def emit_math_subblock_body(fw: MatmulTrisc, plan: MatmulPlan, in0_offset: int, in1_offset: int) -> MatmulTrisc:
  ...
  for reg, extent, block in ((s4, plan.m_extent, plan.out_subblock_h*32),
                             (s5, plan.n_extent, plan.out_subblock_w*32),
                             (s6, plan.k_extent, plan.in0_block_w*32)):
    fw.li(t0, extent // block)
    fw.bge(reg, t0, edge)
...
def emit_pack_tile_to_cb(fw: MatmulTrisc, plan: MatmulPlan, out_cb: int) -> MatmulTrisc:
  ...
  for tile_index in range(plan.out_subblock_num_tiles):
    ...
    fw.li(t2, 0x00FFFF00)
    fw.and_(t1, t1, t2)
    fw.li(t2, TTSETDMAREG(0, 0, 0, 24))  # SETDMAREG[24] base; low addr bits added in
    fw.add(t1, t1, t2)
    fw.write32(TensixRegs.INSTRN_BUF_BASE, t1)
    fw.srli(t1, s0, 16)
    fw.slli(t1, t1, 8)
    fw.li(t2, 0x00800000)
    fw.or_(t1, t1, t2)
    fw.li(t2, TTSETDMAREG(0, 0, 0, 25))  # SETDMAREG[25] base; high addr bits added in
    fw.add(t1, t1, t2)
    fw.write32(TensixRegs.INSTRN_BUF_BASE, t1)
  ...
  fw.read32(t1, fw.data["dest_offset_id"])
  fw.andi(t2, t1, 1)
  fw.li(t3, TTZEROACC(2, 0, 0, 1))  # ZEROACC base; dest-offset parity bit added in
  fw.add(t2, t2, t3)
...
def validate(a_ref: np.ndarray, b_ref: np.ndarray, c_raw: bytes, M: int, N: int, Mp: int, Np: int) -> tuple[float, float]:
  c_full = (np.frombuffer(c_raw, dtype="<f2").astype(np.float32).reshape(Mp,Np) if OUTPUT_DTYPE == Dtype.Float16 else from_bf16_device_bytes(c_raw, (Mp, Np)))
  c_got = c_full[:M, :N]
