...
class Noc:
  def noc_coord(self, out: Reg, x: int | Reg, y: int | Reg, *, tmp: Reg = t0):
    if (isinstance(x, int) and not isinstance(x, Reg)) and (isinstance(y, int) and not isinstance(y, Reg)):
      return self.li(out, noc_xy(x, y))
    if (isinstance(y, int) and not isinstance(y, Reg)):
      self.li(out, y)
    else:
      self.mv(out, y)
    self.slli(out, out, 6)
    if (isinstance(x, int) and not isinstance(x, Reg)):
      self.li(tmp, x)
  ...
  def noc_mcast_coord(self, out: Reg, x_start: int | Reg, y_start: int | Reg,
    ...
    if (isinstance(x_start, int) and not isinstance(x_start, Reg)) and (isinstance(y_start, int) and not isinstance(y_start, Reg)):
  ...
  def sem_addr(self, sem_l1_base: int, sem_id: int | Reg, *, out: Reg = t6, tmp: Reg = t0):
    if (isinstance(sem_id, int) and not isinstance(sem_id, Reg)):
  ...
  def noc_semaphore_set(self, sem_addr: Reg, value: int | Reg, *, tmp: Reg = t0):
    if (isinstance(value, int) and not isinstance(value, Reg)):
      self.li(tmp, value)
      value = tmp
  ...
  def dram_tile_addr_from(self, table_base: int, noc_table_offset: int | Reg = 0, *, tile_bytes=2048):
    self.mv(t0, a1)
    self.remu(a1, t0, a2)
    self.divu(t0, t0, a2)
    self.slli(t0, t0, tile_bytes.bit_length() - 1)
    self.add(a0, a0, t0)
    if (isinstance(noc_table_offset, int) and not isinstance(noc_table_offset, Reg)):
  ...
  def noc_semaphore_set_multicast(self, noc: int, buf: int, sem_addr: Reg, sem_coord: Reg,
                                  ...
                                  a: Reg = t0, v: Reg = t1):
    if not (isinstance(value, int) and not isinstance(value, Reg)):
      self.sw(value, sem_addr, 0)
    else:
      self.li(v, value)
