...
def _pack_data_format(dtype: Dtype) -> int:
  return _PACK_DISABLE_ZERO_COMPRESS | (dtype.value << 4) | ((1 if asm.CONTEXT.get("fp8") else dtype.value) << 8)
...
class Pack:
  ...
  def _alu_acc_rmw(self, k):
    k.emit(TTATGETM(0))
    for inst in (
      TTRMWCIB3(Mask=0x1E, Data=0x02 if asm.CONTEXT.get("fp8") else 0x0A, CfgRegAddr=Cfg.ALU.addr32),
