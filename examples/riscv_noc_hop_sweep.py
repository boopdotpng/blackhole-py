...
def append_report(path: Path, *, iterations: int, runs: list[HopRun]):
  ...
  with path.open("a", encoding="utf-8") as f:
    ...
    f.write("- Traffic: same-row peer-tile L1 unicast only; no DRAM writes and no multicast\n")
    f.write("- Direction policy: NoC0 sweeps peers to the right of the source; NoC1 sweeps peers to the left\n\n")
