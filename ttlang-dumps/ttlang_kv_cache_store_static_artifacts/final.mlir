module attributes {ttl.launch_grid = [2, 8], ttl.pipe_sync_semaphore_count = 0 : i64, ttl.target_arch = "blackhole"} {
  ...
  func.func @dm_read() attributes {ttkernel.arg_spec = #ttkernel.arg_spec< ct_args = [<arg_type = buffer_address, operand_index = 0>, <arg_type = buffer_address, operand_index = 1>]>, ttkernel.thread = #ttkernel.thread<noc>, ttl.base_cta_index = 4 : i32, ttl.crta_indices = [0 : i32, 1 : i32], ttl.enable_fpu_binary_ops = true} {
    ...
    %8 = emitc.literal "get_compile_time_arg_val(1)" {ttkernel.cb_ctarg_idx = 1 : i32} : ui32
    emitc.verbatim "CircularBuffer cb_ctarg_1({});" args %8 : ui32
    ...
    emitc.verbatim "noc0.async_read({}, CoreLocalMem<uint32_t>({}), {}.get_aligned_page_size(), {{.page_id = static_cast<uint32_t>({})}, {{});" args %13, %14, %13, %18 : !emitc.opaque<"TensorAccessor">, i32, !emitc.opaque<"TensorAccessor">, i32
    ...
    %22 = emitc.literal "cb_ctarg_1.get_write_ptr()" : i32
    emitc.verbatim "noc0.async_read({}, CoreLocalMem<uint32_t>({}), {}.get_aligned_page_size(), {{.page_id = static_cast<uint32_t>({})}, {{});" args %21, %22, %21, %18 : !emitc.opaque<"TensorAccessor">, i32, !emitc.opaque<"TensorAccessor">, i32
    emitc.verbatim "noc0.async_read_barrier();"
  ...
  func.func @dm_write() attributes {ttkernel.arg_spec = #ttkernel.arg_spec< ct_args = [<arg_type = buffer_address, operand_index = 0>, <arg_type = buffer_address, operand_index = 1>]>, ttkernel.thread = #ttkernel.thread<noc>, ttl.base_cta_index = 4 : i32, ttl.crta_indices = [2 : i32, 3 : i32], ttl.enable_fpu_binary_ops = true} {
    ...
    %3 = "emitc.constant"() <{value = 1 : i8}> : () -> i8
    ...
    %10 = "emitc.constant"() <{value = #emitc.opaque<"get_absolute_logical_x()">}> : () -> !emitc.size_t
    %11 = "emitc.constant"() <{value = #emitc.opaque<"get_absolute_logical_y()">}> : () -> !emitc.size_t
    emitc.verbatim "cb_ctarg_2.wait_front({});" args %7 : i32
    ...
    %15 = emitc.literal "cb_ctarg_2.get_read_ptr()" : i32
    %16 = emitc.mul %11, %1 : (!emitc.size_t, !emitc.size_t) -> !emitc.size_t
    %17 = emitc.add %16, %0 : (!emitc.size_t, !emitc.size_t) -> !emitc.size_t
    %18 = emitc.add %17, %10 : (!emitc.size_t, !emitc.size_t) -> !emitc.size_t
    %19 = emitc.cast %18 : !emitc.size_t to !emitc.ptrdiff_t
    %20 = emitc.cast %19 : !emitc.ptrdiff_t to i32
    emitc.verbatim "noc1.async_write(CoreLocalMem<uint32_t>({}), {}, {}.get_aligned_page_size(), {{} , {{.page_id = static_cast<uint32_t>({})});" args %15, %14, %14, %20 : i32, !emitc.opaque<"TensorAccessor">, !emitc.opaque<"TensorAccessor">, i32
    emitc.verbatim "noc1.async_write_barrier();"
    emitc.verbatim "cb_ctarg_2.pop_front({});" args %7 : i32
    emitc.verbatim "cb_ctarg_3.wait_front({});" args %7 : i32
    %21 = emitc.call_opaque "get_common_arg_val"(%2) <{template_args = [#emitc.opaque<"uint32_t">]}> : (!emitc.size_t) -> i32
    ...
    %22 = emitc.literal "tensor_accessor_args_1" : !emitc.opaque<"TensorAccessorArgs">
    %23 = emitc.call_opaque "TensorAccessor"(%22, %21, %5)  : (!emitc.opaque<"TensorAccessorArgs">, i32, i32) -> !emitc.opaque<"TensorAccessor">
    %24 = emitc.literal "cb_ctarg_3.get_read_ptr()" : i32
    emitc.verbatim "noc1.async_write(CoreLocalMem<uint32_t>({}), {}, {}.get_aligned_page_size(), {{} , {{.page_id = static_cast<uint32_t>({})});" args %24, %23, %23, %20 : i32, !emitc.opaque<"TensorAccessor">, !emitc.opaque<"TensorAccessor">, i32
    emitc.verbatim "noc1.async_write_barrier();"
    emitc.verbatim "cb_ctarg_3.pop_front({});" args %7 : i32
    return
  }
}
