module attributes {ttl.launch_grid = [1, 1], ttl.target_arch = "blackhole"} {
  ...
  func.func @dm_read(%arg0: tensor<1x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [32, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>>, %arg1: tensor<1x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [32, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>>, %arg2: tensor<32x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [1024, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>>) attributes {ttl.base_cta_index = 8 : i32, ttl.crta_indices = [1 : i32, 2 : i32, 0 : i32], ttl.kernel_thread = #ttkernel.thread<noc>, ttl.noc_index = 0 : i32} {
    %0 = ttl.bind_cb{cb_index = 3, block_count = 2} : <[1, 1], !ttcore.tile<32x32, bf16>, 2>
    %1 = ttl.bind_cb{cb_index = 2, block_count = 2} : <[1, 1], !ttcore.tile<32x32, bf16>, 2>
    %c1 = arith.constant 1 : index
    ...
    scf.for %arg3 = %c0 to %c32 step %c1_0 {
      ...
      %9 = ttl.tensor_slice %arg2[%arg3, %c0_1] : tensor<32x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [1024, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>> -> tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [1024, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>>
      %10 = ttl.copy %9, %5 : (tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [1024, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>>, !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 2>) -> !ttl.transfer_handle<read>
      ...
      %13 = ttl.tensor_slice %arg2[%arg3, %c1_2] : tensor<32x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [1024, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>> -> tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [1024, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>>
      %14 = ttl.copy %13, %4 : (tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [1024, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>>, !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 2>) -> !ttl.transfer_handle<read>
  ...
  func.func @dm_write(%arg0: tensor<32x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [1024, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>>) attributes {ttl.base_cta_index = 8 : i32, ttl.crta_indices = [3 : i32], ttl.kernel_thread = #ttkernel.thread<noc>, ttl.noc_index = 1 : i32} {
    ...
    scf.for %arg1 = %c0 to %c32 step %c1 {
      ...
      %4 = ttl.tensor_slice %arg0[%arg1, %c0_0] : tensor<32x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [1024, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>> -> tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [1024, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>>
      %5 = ttl.copy %1, %4 : (!ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 2>, tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [1024, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>>) -> !ttl.transfer_handle<write>
      ...
      %8 = ttl.tensor_slice %arg0[%arg1, %c1_1] : tensor<32x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [1024, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>> -> tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [1024, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>>
      %9 = ttl.copy %0, %8 : (!ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 2>, tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [1024, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [1, 1], memory = interleaved>>) -> !ttl.transfer_handle<write>
