module attributes {ttl.launch_grid = [1, 32], ttl.target_arch = "blackhole"} {
  ...
  func.func @dm_read(%arg0: tensor<32x4x!ttcore.tile<32x32, f32>, #ttl.layout<shape = [1024, 128], element_type = !ttcore.tile<32x32, f32>, buffer = l1, grid = [32, 1], memory = interleaved>>, %arg1: tensor<32x4x!ttcore.tile<32x32, f32>, #ttl.layout<shape = [1024, 128], element_type = !ttcore.tile<32x32, f32>, buffer = l1, grid = [32, 1], memory = interleaved>>) attributes {ttl.base_cta_index = 3 : i32, ttl.crta_indices = [1 : i32, 0 : i32], ttl.kernel_thread = #ttkernel.thread<noc>, ttl.noc_index = 0 : i32} {
    %0 = ttl.bind_cb{cb_index = 1, block_count = 2} : <[1, 4], !ttcore.tile<32x32, f32>, 2>
    ...
    %6 = ttl.cb_reserve %0 : <[1, 4], !ttcore.tile<32x32, f32>, 2> -> tensor<1x4x!ttcore.tile<32x32, f32>>
    %7 = ttl.attach_cb %6, %0 : (tensor<1x4x!ttcore.tile<32x32, f32>>, !ttl.cb<[1, 4], !ttcore.tile<32x32, f32>, 2>) -> tensor<1x4x!ttcore.tile<32x32, f32>>
    %c0 = arith.constant 0 : index
    %8 = ttl.tensor_slice %arg1[%3, %c0] : tensor<32x4x!ttcore.tile<32x32, f32>, #ttl.layout<shape = [1024, 128], element_type = !ttcore.tile<32x32, f32>, buffer = l1, grid = [32, 1], memory = interleaved>> -> tensor<1x4x!ttcore.tile<32x32, f32>, #ttl.layout<shape = [1024, 128], element_type = !ttcore.tile<32x32, f32>, buffer = l1, grid = [32, 1], memory = interleaved>>
    %9 = ttl.copy %8, %1 : (tensor<1x4x!ttcore.tile<32x32, f32>, #ttl.layout<shape = [1024, 128], element_type = !ttcore.tile<32x32, f32>, buffer = l1, grid = [32, 1], memory = interleaved>>, !ttl.cb<[1, 4], !ttcore.tile<32x32, f32>, 2>) -> !ttl.transfer_handle<read>
    ...
    %11 = ttl.copy %10, %0 : (tensor<1x4x!ttcore.tile<32x32, f32>, #ttl.layout<shape = [1024, 128], element_type = !ttcore.tile<32x32, f32>, buffer = l1, grid = [32, 1], memory = interleaved>>, !ttl.cb<[1, 4], !ttcore.tile<32x32, f32>, 2>) -> !ttl.transfer_handle<read>
    ttl.wait %11 : !ttl.transfer_handle<read>
    ttl.cb_push %0 : <[1, 4], !ttcore.tile<32x32, f32>, 2>
    ttl.cb_push %1 : <[1, 4], !ttcore.tile<32x32, f32>, 2>
    return
  }
  func.func @dm_write(%arg0: tensor<32x4x!ttcore.tile<32x32, f32>, #ttl.layout<shape = [1024, 128], element_type = !ttcore.tile<32x32, f32>, buffer = l1, grid = [32, 1], memory = interleaved>>) attributes {ttl.base_cta_index = 3 : i32, ttl.crta_indices = [2 : i32], ttl.kernel_thread = #ttkernel.thread<noc>, ttl.noc_index = 1 : i32} {
