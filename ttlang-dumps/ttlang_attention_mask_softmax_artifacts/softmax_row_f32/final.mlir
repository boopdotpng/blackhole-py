module attributes {ttl.compiler_allocated_dfbs = [{block_count = 2 : i32, dfb_index = 3 : i32, element_type = !ttcore.tile<32x32, f32>, num_tiles = 1 : i32}, {block_count = 2 : i32, dfb_index = 4 : i32, element_type = !ttcore.tile<32x32, f32>, num_tiles = 1 : i32}, {block_count = 2 : i32, dfb_index = 5 : i32, element_type = !ttcore.tile<32x32, f32>, num_tiles = 1 : i32}, {block_count = 2 : i32, dfb_index = 6 : i32, element_type = !ttcore.tile<32x32, f32>, num_tiles = 4 : i32}], ttl.launch_grid = [1, 32], ttl.pipe_sync_semaphore_count = 0 : i64, ttl.target_arch = "blackhole"} {
  ...
  func.func @dm_read() attributes {dst_full_sync_en = true, fp32_dest_acc_en = true, ttkernel.arg_spec = #ttkernel.arg_spec< ct_args = [<arg_type = buffer_address, operand_index = 0>]>, ttkernel.thread = #ttkernel.thread<noc>, ttl.base_cta_index = 7 : i32, ttl.crta_indices = [0 : i32], ttl.enable_fpu_binary_ops = true} {
    ...
    %11 = emitc.call_opaque "get_common_arg_val"(%7) <{template_args = [#emitc.opaque<"uint32_t">]}> : (!emitc.size_t) -> i32
    ...
    emitc.for %arg0 = %7 to %0 step %1  : !emitc.size_t {
      %22 = mul %10, %0 : (!emitc.size_t, !emitc.size_t) -> !emitc.size_t
      %23 = add %22, %arg0 : (!emitc.size_t, !emitc.size_t) -> !emitc.size_t
      %24 = mul %arg0, %3 : (!emitc.size_t, !emitc.size_t) -> !emitc.size_t
      %25 = add %16, %24 : (!emitc.size_t, !emitc.size_t) -> !emitc.size_t
      %26 = cast %23 : !emitc.size_t to !emitc.ptrdiff_t
      %27 = cast %26 : !emitc.ptrdiff_t to i32
      %28 = cast %25 : !emitc.size_t to !emitc.ptrdiff_t
      %29 = cast %28 : !emitc.ptrdiff_t to i32
      verbatim "noc0.async_read({}, CoreLocalMem<uint32_t>({}), {}.get_aligned_page_size(), {{.page_id = static_cast<uint32_t>({})}, {{});" args %13, %29, %13, %27 : !emitc.opaque<"TensorAccessor">, i32, !emitc.opaque<"TensorAccessor">, i32
    }
    ...
    %17 = emitc.literal "tensor_accessor_args_1" : !emitc.opaque<"TensorAccessorArgs">
    %18 = emitc.call_opaque "TensorAccessor"(%17, %11, %5)  : (!emitc.opaque<"TensorAccessorArgs">, i32, i32) -> !emitc.opaque<"TensorAccessor">
    %19 = emitc.literal "cb_ctarg_1.get_write_ptr()" : i32
    %20 = emitc.cast %19 : i32 to !emitc.ptrdiff_t
    %21 = emitc.cast %20 : !emitc.ptrdiff_t to !emitc.size_t
    emitc.for %arg0 = %7 to %0 step %1  : !emitc.size_t {
      %22 = mul %10, %0 : (!emitc.size_t, !emitc.size_t) -> !emitc.size_t
      %23 = add %22, %arg0 : (!emitc.size_t, !emitc.size_t) -> !emitc.size_t
      %24 = mul %arg0, %3 : (!emitc.size_t, !emitc.size_t) -> !emitc.size_t
      %25 = add %21, %24 : (!emitc.size_t, !emitc.size_t) -> !emitc.size_t
      %26 = cast %23 : !emitc.size_t to !emitc.ptrdiff_t
      %27 = cast %26 : !emitc.ptrdiff_t to i32
      %28 = cast %25 : !emitc.size_t to !emitc.ptrdiff_t
      %29 = cast %28 : !emitc.ptrdiff_t to i32
      verbatim "noc0.async_read({}, CoreLocalMem<uint32_t>({}), {}.get_aligned_page_size(), {{.page_id = static_cast<uint32_t>({})}, {{});" args %18, %29, %18, %27 : !emitc.opaque<"TensorAccessor">, i32, !emitc.opaque<"TensorAccessor">, i32
    }
    emitc.verbatim "noc0.async_read_barrier();"
