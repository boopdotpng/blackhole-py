module attributes {ttl.launch_grid = [2, 32]} {
  ...
  func.func @dm_read(%arg0: tensor<8x256x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [8, 8192, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>>, %arg1: tensor<8x256x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [8, 8192, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>>) attributes {ttl.base_cta_index = 4 : i32, ttl.crta_indices = [0 : i32, 1 : i32], ttl.kernel_thread = #ttkernel.thread<noc>, ttl.noc_index = 0 : i32} {
    %0 = ttl.bind_cb{cb_index = 0, block_count = 2} : <[1, 1], !ttcore.tile<32x32, bf16>, 2>
    ...
    %9 = ttl.tensor_slice %arg0[%5, %8, %2] : tensor<8x256x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [8, 8192, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>> -> tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [8, 8192, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>>
    %10 = ttl.copy %9, %0 : (tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [8, 8192, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>>, !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 2>) -> !ttl.transfer_handle<read>
    ...
    %14 = ttl.tensor_slice %arg1[%5, %13, %2] : tensor<8x256x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [8, 8192, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>> -> tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [8, 8192, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>>
    %15 = ttl.copy %14, %1 : (tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [8, 8192, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>>, !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 2>) -> !ttl.transfer_handle<read>
    ttl.wait %15 : !ttl.transfer_handle<read>
    ttl.cb_push %1 : <[1, 1], !ttcore.tile<32x32, bf16>, 2>
    return
  }
  func.func @dm_write(%arg0: tensor<32x1x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [32, 32, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>>, %arg1: tensor<32x1x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [32, 32, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>>) attributes {ttl.base_cta_index = 4 : i32, ttl.crta_indices = [2 : i32, 3 : i32], ttl.kernel_thread = #ttkernel.thread<noc>, ttl.noc_index = 1 : i32} {
    ...
    %6 = ttl.tensor_slice %arg0[%3, %c0, %2] : tensor<32x1x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [32, 32, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>> -> tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [32, 32, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>>
    %7 = ttl.copy %0, %6 : (!ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 2>, tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [32, 32, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>>) -> !ttl.transfer_handle<write>
    ...
    %10 = ttl.tensor_slice %arg1[%3, %c0_0, %2] : tensor<32x1x2x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [32, 32, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>> -> tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [32, 32, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>>
    %11 = ttl.copy %1, %10 : (!ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 2>, tensor<1x1x!ttcore.tile<32x32, bf16>, #ttl.layout<shape = [32, 32, 64], element_type = !ttcore.tile<32x32, bf16>, buffer = l1, grid = [32, 2], memory = interleaved>>) -> !ttl.transfer_handle<write>
