...
`emu/clauses/<doc>.md` ledger via the `@spec("CLAUSE.ID", ...)`
decorator. Each ledger cites the authoritative prose spec in `emu/specs/<doc>.md`.
...
def pytest_configure(config):
  config.addinivalue_line(
    "markers",
    "spec(*clause_ids): pin a test to spec clauses from emu/clauses/<doc>.md",
  )
