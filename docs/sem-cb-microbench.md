...
useful as constants for `examples/program_timing_model.py`.

The harness lives in `examples/microbench_sem_cb.py`.
...
```sh
PYTHONPATH=. TT_USB=1 /home/boop/tenstorrent/.venv/bin/python3 examples/microbench_sem_cb.py --iters 1000 --release-delay 64
```
...
`examples/program_timing_model.py`. Blocking rows should remain separate model
