...
The initial harness lives in `examples/riscv_core_bench.py`.

Special cases that need separate handling (`jalr`, CSR reads, load/store width
variants, and all branch opcodes) live in
`examples/riscv_special_instr_bench.py` and are reported in
`docs/riscv-special-instr-microbench.md`.

Memory-specific probes live in `examples/riscv_memory_bench.py` and are reported
in `docs/riscv-memory-microbench.md`.
...
```sh
PYTHONPATH=. TT_USB=1 /home/boop/tenstorrent/.venv/bin/python3 examples/riscv_core_bench.py --iters 10000
```
