...
Decode optimizations now reach **152 tok/s with 83 launches per token** on the
tested P150A, up from 139 tok/s and 212 launches before the latest round. The
default uses 16 attention workers and preserves the reference BF16 results.
See [fusion measurements and validation](decode-fusion-round2.md),
...
## Experimental distributed bring-up

[Independent ERISC protocol, mesh design, card 1 link observations, and test
plan](docs/distributed/README.md). This adds our own raw-Ethernet packet codec,
C transport building blocks, and CPU-tested planning tools; multi-card inference
and peer payload testing are pending. No tt-metal code dependency.
See the [two-card Llama TP design and speed estimate](docs/distributed/tensor-parallel.md).
