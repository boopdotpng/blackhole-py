...
The default in this copied repository is Neural Magic's calibrated
[RedHatAI/Meta-Llama-3-8B-Instruct-FP8](https://huggingface.co/RedHatAI/Meta-Llama-3-8B-Instruct-FP8),
revision `c5c6b5700a4178ef1fdae2ae37827382b90eb400`, downloaded to
`weights-published-fp8/` by `scripts/download_fp8_weights.py`.
The original BF16 provenance above applies to the comparison checkpoint in
`weights/`. The publisher uses static FP8 activation and weight scales and
retains BF16 embeddings, normalization weights and LM head. Our SHA-256 checks
confirmed identical BF16 embeddings, LM head, and two sampled normalization
weights between these distributions; see `validation/checkpoint-comparison.json`.
The published FP8 checkpoint contains 291 model tensors plus 448 scalar scales.
The model remains governed by the Llama 3 license.
