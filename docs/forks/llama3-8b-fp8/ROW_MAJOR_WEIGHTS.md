...
Card 1, 64 generated tokens with identical reference history:
47.75 tok/s tiled, 47.91 tok/s raw (+0.35%).
All token choices matched; maximum sampled logit relative RMS error was
0.000379605%. See [report](validation/row-major-weights.json).

The broader CPU suite retains one pre-existing failure in
`test_fp8.FP8Test.test_conversion_matches_torch`: the independent large-array
FP8 encoder comparison fails in this environment. The FP8 conversion module
was not changed by this port. Layout tests, device byte readback (including
FP8), and the published-checkpoint hardware comparison pass.

The alternate `weights-fp8` experiment (FP8 dense intermediates) also passed
a 32-token same-history comparison: 51.35 to 51.54 tok/s, all token choices
matched, and maximum sampled logit RMS difference 0.00181775%. See
[alternate-format report](validation/row-major-internal-fp8.json). The numerical
comparison is against that experiment's own pre-change behavior.
