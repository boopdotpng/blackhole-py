"""Shared device runtime: raw launches, tensor programs, uploads and traces."""
...
from firmware.consts import (
  Firmware, FirmwareControl, HOST_TRACE_SIZE, KERNEL_ROLES, RunState,
  TensixL1, TensixMMIO,
)
from pcie import Allocator, PCIDevice, TLBWindow
from program import (
  PARAM_BASE, RETURN_KERNEL, Buffer, Dram, Program, TensorProgram, rectangles,
)
...
class Device:
  ...
  def boot(self, images=None):
    ...
    with TLBWindow(self.pcie.fd, self.pcie.cores[0]) as window:
      ...
      self.cq = self.command_queue_type(self.pcie, trace_size=self.trace_size)
      for core in (self.pcie.prefetch_core, self.pcie.dispatch_core, self.pcie.dram_core):
        window.target(0, core)
...
class TensorDevice(Device):
  """Tensor queues and resident traces on the shared raw device runtime."""
  trace_size = HOST_TRACE_SIZE
  ...
  def queue(self, program: TensorProgram, params=None, report=True):
  ...
  def cache_kernels(self, programs, timeout=30.0):
    ...
    if commands:
      noop = TensorProgram(cores, images={})
      self.cq.submit(
  ...
  def run(self, *programs: TensorProgram, params=None, timeout=10.0):
    self._check_upload_idle()
