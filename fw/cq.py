...
# CQ ABI: cq.py (host encoder) is the single source of truth. Alias its public
# names here so the on-device firmware can never drift from the host layout.
PREFETCH_Q_RD_PTR = cq.CQ_PREFETCH_Q_RD_PTR
PREFETCH_Q_PCIE_RD = cq.CQ_PREFETCH_Q_PCIE_RD
PREFETCH_Q_BASE = cq.CQ_PREFETCH_Q_BASE
PREFETCH_Q_SIZE = cq.CQ_PREFETCH_Q_SIZE
PREFETCH_Q_END = PREFETCH_Q_BASE + PREFETCH_Q_SIZE
PREFETCH_Q_ENTRY_SIZE = cq.CQ_PREFETCH_Q_ENTRY_SZ
HOST_ISSUE_BASE = 0x40000100
DISPATCH_CB_BASE = 0x1A000
DISPATCH_CB_PAGES = cq.CQ_DISPATCH_CB_PAGES  # ring fills 0x1A000..0x15A000; CMDDAT sits above
...
# Command IDs shared with the host encoder are aliased from cq.py; the
# prefetch-only IDs (NOFLUSH/EXEC_BUF_END/STALL/TERMINATE) live solely in the
# device firmware and stay local.
CQ_PREFETCH_CMD_RELAY_INLINE = cq.CMD_RELAY_INLINE
CQ_PREFETCH_CMD_RELAY_INLINE_NOFLUSH = 6
CQ_PREFETCH_CMD_EXEC_BUF_END = 8
CQ_PREFETCH_CMD_STALL = 9
CQ_PREFETCH_CMD_TERMINATE = 11
CQ_DISPATCH_CMD_WRITE_LINEAR_H_HOST = cq.CMD_WRITE_LINEAR_HOST
CQ_DISPATCH_CMD_WRITE_PACKED = cq.CMD_WRITE_PACKED
CQ_DISPATCH_CMD_WRITE_PACKED_LARGE = cq.CMD_WRITE_PACKED_LARGE
CQ_DISPATCH_CMD_WAIT = cq.CMD_WAIT
CQ_DISPATCH_CMD_TERMINATE = 13
CQ_DISPATCH_CMD_SEND_GO_SIGNAL = cq.CMD_GO_SIGNAL
CQ_DISPATCH_SET_GO_SIGNAL_NOC_DATA = cq.CMD_SET_GO_NOC_DATA
CQ_DISPATCH_CMD_TIMESTAMP = cq.CMD_TIMESTAMP
CQ_WAIT_BARRIER = cq.CQ_WAIT_BARRIER
CQ_WAIT_WAIT_STREAM = cq.CQ_WAIT_WAIT_STREAM
CQ_WAIT_CLEAR_STREAM = cq.CQ_WAIT_CLEAR_STREAM
CQ_PACKED_NO_STRIDE = cq.CQ_PACKED_NO_STRIDE
CQ_PACKED_LARGE_UNLINK = cq.CQ_PACKED_LARGE_UNLINK
GO_NO_MULTICAST = cq.GO_NO_MULTICAST
...
CQ_DEBUG = 0x19000
GO_SIGNAL_NOC_DATA = CQ_DEBUG + 0x200  # was 0xB0000, now inside the grown ring
