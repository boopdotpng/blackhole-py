...
TT_INLINE void zero_words(u32 address, u32 count) {
  ...
  if (count) do { *words++ = 0; } while (--count);
}
...
static void noc_submit(
  ...
  command[11] = 0;
  /* Flush command-register stores before SEND, and SEND before polling.
   * Compiled C can reach the status load before buffered MMIO stores land. */
  fence();
  command[16] = 1;
  fence();
  while (command[16] != 0) {}
...
#ifdef TT_WORKER_INDEX
TT_INLINE __attribute__((noreturn)) void run_worker_kernel(void) {
  __asm__ volatile("jr %0" :: "r"(mmio_read32(L1_WORKER_ENTRY_BASE + TT_WORKER_INDEX * 4)) : "memory");
...
__asm__(
  ...
  "li gp, " TT_STRINGIFY(TT_FW_GLOBAL_POINTER) "\n"
#endif
  "j firmware_resume_after_kernel\n"
#endif
  ".popsection\n"
);
#endif
