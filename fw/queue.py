from fw.abi import BOOT_PCIE_MID
from asm import Asm
from fw.abi import (
...
def _emit_prefetch(fw, state):
  ...
  noc = fw.noc
  pcie_mid = fw.reg()
  fw.read(pcie_mid, BOOT_PCIE_MID)
...
def _emit_dispatch(fw, state):
  ...
  fw.li(s4, int(Op.TIMESTAMP)); fw.beq(s3, s4, "dram_enqueue")
  fw.li(s4, int(Op.DMA)); fw.beq(s3, s4, "dram_enqueue")
  ...
  # Poll a full 64-bit timeline. The host re-arms it only after the previous
  # submission completed. Each NoC read waits for its response before compare.
  fw.label("wait_signal")
  fw.lw(s4, s0, PacketLayout.ADDRESS)
  fw.lw(s5, s0, PacketLayout.DATA_SIZE)
  noc.read(s4, CQConfig.PCIE_COORD, DISPATCH_GO, 8, source_middle_address=s5)
  fw.fence()
  fw.read(s6, DISPATCH_GO); fw.read(s7, DISPATCH_GO + 4)
  fw.lw(s8, s0, 16); fw.lw(s9, s0, 20)
  fw.lw(s10, s0, 24)
  fw.bne(s10, R.ZERO, "wait_equal")
  fw.bltu(s9, s7, "command_done")
  fw.bltu(s7, s9, "wait_signal")
  fw.bgeu(s6, s8, "command_done")
  fw.j("wait_signal")
  fw.label("wait_equal")
  fw.bne(s7, s9, "wait_signal")
  fw.bne(s6, s8, "wait_signal")
  fw.j("command_done")
