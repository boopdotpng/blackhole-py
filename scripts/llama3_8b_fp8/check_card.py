...
from examples.diagnostics.llama3_8b_fp8.profile_llama3 import ProfileRuntime, profile
...
model = AutoModelForCausalLM.from_pretrained('weights/llama3-8b-fp8', local_files_only=True, dtype=torch.bfloat16, attn_implementation='eager').eval()
with torch.inference_mode():
  cpu_logits = model(torch.tensor([ids])).logits[0].float().numpy()
del model
print('Loading card', flush=True)
runtime = ProfileRuntime('weights/llama3-8b-fp8', args.device)
...
try:
  ...
  import examples.diagnostics.llama3_8b_fp8.profile_llama3 as profiling
  profiling.ProfileRuntime = lambda *a, **kw: runtime
  output = profile(argparse.Namespace(safetensor='weights/llama3-8b-fp8', device=args.device, attention_cores=32, contexts=[32,128]))
