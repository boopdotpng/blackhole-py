"""Raw image programs and tensor kernel builders sharing CQ image lowering."""
...
from asm import Asm
from cq import McastWrite, Run, UnicastWrite, rectangles
from firmware.consts import Firmware, KERNEL_ROLES, MAX_WRITE_SIZE, TensixL1
...
class TensorProgram:
  def __init__(self, cores, *parameters, fp32_dst=False, images=None):
    ...
    self.roles = {
      role: Asm(role, param_slots=self._param_slots, physical_regs=True)
      for role in KERNEL_ROLES
    }
  ...
  def static_commands(self):
    ...
    for role in KERNEL_ROLES:
      ...
      for image, cores in groups.items():
        commands.extend(_writes(cores, TensixL1.WORKER_TEXT_BASE[role], image))
