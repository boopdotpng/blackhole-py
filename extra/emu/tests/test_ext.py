"""Tests for M, Zba, and Zbb extension support in the emulator."""
...
def test_mul_overflow():
    """Lower 32 bits of a large product."""
    core = run([
...
def test_mulhu():
    """Upper 32 bits of unsigned multiply — the divide-by-7 magic constant."""
    core = run([
...
def test_mulhu_div7_pattern():
    """Full divide-by-7 via mulhu + srli as used in add1_brisc."""
    magic = 0x92492493
...
def test_divu_by_zero():
    """RISC-V spec: DIVU by zero returns 0xFFFFFFFF."""
    core = run([
...
def test_remu_by_zero():
    """RISC-V spec: REMU by zero returns the dividend."""
    core = run([
        dsl.ADDI(dsl.a0, dsl.zero, 42),
...
def test_sh2add_address_calc():
    """sh2add used for word-array indexing as in add1 kernels."""
    core = run([
...
def test_add1_brisc_zext_mulhu_sequence():
    """
    The pattern from add1_brisc.S:
      add   a5, a5, a4
      zext.h a5, a5
      ...
      mulhu a5, s7, s3   (divide-by-7)
      srli  a5, a5, 2
      sh2add a3, a4, s3  (word-index into bank table)
    """
    magic = 0x92492493
    tile = 21
    base = 0x2000
